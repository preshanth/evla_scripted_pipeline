...
class TestPolarizationImports:
    ...
    def test_polarization_utils_import(self):
        ...
        try:
            from evla_pipe.pol_setjy_utils import (
                fit_polarization_polynomials,
                get_polcal_data,
            )
...
class TestPolarizationConfig:
    ...
    def test_pol_calibrator_creation(self):
        ...
        try:
            ...
            calibrator = PolCalibrator(name="3C286", field_id=0, spws=[0])
            assert calibrator.name == "3C286"
            assert calibrator.field_id == 0

        except (ImportError, TypeError):
            pytest.skip(
                "PolCalibrator not available without CASA or requires more parameters"
            )
...
class TestPipelineIntegration:
    ...
    def test_continuum_function_signature(self):
        ...
        assert not default_val, "Default value for enable_polarization should be False"
...
class TestCLIIntegration:
    ...
    def test_cli_polarization_flags(self):
        ...
        assert args.polarization

        # Test default value
        args_default = parser.parse_args(["test.ms"])
        assert not args_default.polarization
...
def test_polarization_workflow_structure(sample_polarization_context):
    ...
    # Test initial state
    assert context["do_pol"]
    assert not context["polarization_calibrated"]
    ...
    # Test final state
    assert context["polarization_calibrated"]
