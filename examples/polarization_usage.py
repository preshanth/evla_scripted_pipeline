...
def example_object_oriented(msname):
    ...
    try:
        with PolarizationCalibrator(msname, config) as polcal:
            ...
            print(f"Found calibrators: {[cal.name for cal in calibrators.values() if cal]}")
...
def example_step_by_step(msname):
    ...
    from evla_pipe.polarization import (
        find_pol_calibrators, set_pol_models, solve_pol_kcross,
        solve_pol_leakage, solve_pol_angle, PolConfig
    )
    ...
    try:
        ...
        polangle_table = solve_pol_angle(msname, angle_cal, [kcross_table, dterms_table], config)
...
def example_pipeline_integration(pipeline_context):
    ...
    if updated_context.get('polarization_success'):
        ...
        print(f"Polarization failed: {updated_context.get('polarization_error', 'Unknown error')}")
