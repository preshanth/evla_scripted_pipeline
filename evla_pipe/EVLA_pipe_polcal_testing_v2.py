...
import numpy as np
import scipy as sp
from casatasks import gaincal, setjy, split
...
def fitterI(freqI_band, a, b):
    ...
    for i in range(len(freqI_band)):
        I = np.append(
            I,
            i_ref
            * (freqI_band[i] / refFreqI)
            ** (a + (b * np.log10(freqI_band[i] / refFreqI))),
        )
    return I
...
def polyFit(polAngleSource, band, refFreq):
    ...
    cal_data2013 = np.genfromtxt(
        "../../EVLA_Scripted_Pipeline/EVLA_SCRIPTED_PIPELINE/data/PolCals_2013_3C48.3C138.3C147.3C286.dat"
    )
    ...
    if "3C48" in polAngleSource:
        ...
        task_logprint(
            "Not evaluating at frequency 3.75 GHz due to null value"
            "for polarization fraction at that frequency."
        )
    elif "3C147" in polAngleSource:
        if (band == "L") or (band == "S"):
            task_logprint(
                "Unable to fit with L or S band for 3C147\n" "Quitting script.\n"
            )
        ...
        task_logprint(
            "This source is not in this table.\n"
    ...
    popt_pf, pcov_pf = sp.optimize.curve_fit(fitter3, x_data, pol / 100.0)
...
if do_pol:
    ...
    plotms(
        ...
        yaxis="amp",
        ydatacolumn="model",
        plotfile=str(polAngleField) + "_ampvsfreq_RR_model.png",
        overwrite=True,
    )
    ...
    for i in range(len(flux_dict[polarization_angle_field_select_string]) - 1):
        ...
        fluxI = np.append(
            fluxI, flux_dict[polarization_angle_field_select_string][str(i)]["fluxd"][0]
        )
    ...
    task_logprint(
        "Please note that assuming this is a continuum scan, so assuming that "
    ...
    for i in range(len(bandList)):
        ...
        if "_L#" in bandList[i]:
            ...
            task_logprint(
                "Unable to do calibration for this " + str(bandList[i]) + "\n"
            )
            quit()
    ...
    for band in bandsList:
        if band == "L":
            ...
            print(
                str(band) + " is not a recognized band for calibration.  Please"
        ...
        task_logprint(
            "len of freqI_band is "
            + str(len(freqI_band))
            + "\ncompared to len of freqI, which is "
            + str(len(freqI))
            + "\n"
        )
        ...
        # polOut =
        coeffs_pf, coeffs_pa, p_ref = polyFit(polAngleField, band, refFreqI / 1e09)
        ...
        plotms(
            ...
            yaxis="amp",
            ydatacolumn="model",
            plotfile=str(polAngleField) + "_ampvsfreq_RL_model.png",
            overwrite=True,
        )
        ...
        plotms(
            ...
            yaxis="phase",
            ydatacolumn="model",
            plotfile=str(polAngleField) + "_phasevsfreq_RL_model.png",
            overwrite=True,
        )
        ...
        plotms(
            ...
            antenna=refAnt,
            coloraxis="corr",
            plotfile=str(polAngleField) + "_delayvsfreq_kcross_sbd_sol.png",
            overwrite=True,
        )
