...
def pipeline_startup(pipeline_context):
    ...
    def task_logprint(msg):
        return logprint(msg, logfileout=str(get_log_path("startup.log")))
    ...
    pipeline_context["scratch"] = pipeline_context.get(
        "scratch", False
    )  # Always False, scratch columns not used
    pipeline_context["do_hanning"] = pipeline_context.get(
        "do_hanning", False
    )  # Default: no Hanning smoothing (use --hanning flag)
    pipeline_context["do_pol"] = pipeline_context.get(
        "do_pol", False
    )  # Default: no polarization calibration (use --polarization flag)
    ...
    pipeline_context["observeDateString"] = pipeline_context.get(
        "observeDateString", "Unknown"
    )
    pipeline_context["pipelineDateString"] = pipeline_context.get(
        "pipelineDateString", datetime.now().strftime("%Y-%m-%d")
    )

    # Log the configuration
    task_logprint("Pipeline configuration:")
