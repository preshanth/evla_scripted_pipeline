...
from evla_pipe import __version_str__, check_casa_version, continuum
...
Examples:
  ...
  %(prog)s --version
    ...
    parser.add_argument(
        "sdm_name", nargs="?", help="SDM directory name (without .ms extension)"
    )
    ...
    parser.add_argument(
        "--skip", metavar="STEP", action="append", help="Skip specific pipeline step(s)"
    )

    parser.add_argument(
        "--restore", metavar="FILE", help="Restore pipeline state from backup file"
    )

    parser.add_argument(
        "--save", metavar="FILE", help="Save pipeline state to backup file"
    )
