...
from evla_pipe.utils import find_EVLA_band, format_qa_status, logprint, runtiming, uniq
...
def get_ms_info(pipeline_context):
    ...
    try:
        ...
        task_logprint(
            f"MS: {basic['nspw']} spws, {basic['nfields']} fields, {basic['nantennas']} antennas, {basic['nscans']} scans"
        )
        ...
        for spwid in range(basic["nspw"]):
            ...
            spw_info["name"] = (
                msmd.namesforspws([spwid])[0]
                if msmd.namesforspws([spwid])
                else f"spw{spwid}"
            )
            ...
            spw_info["centerfreq_hz"] = (
                spw_info["reffreq_hz"] + spw_info["bandwidth_hz"] / 2
            )
        ...
        for fieldid in range(basic["nfields"]):
            ...
            field_info["name"] = (
                field_names[fieldid]
                if fieldid < len(field_names)
                else f"field{fieldid}"
            )
        ...
        for antid in range(basic["nantennas"]):
            ant_info = {}
            ant_info["name"] = (
                antenna_names[antid] if antid < len(antenna_names) else f"ant{antid}"
            )
            ...
            ant_info["station"] = (
                msmd.antennastations([antid])[0]
                if msmd.antennastations([antid])
                else f"station{antid}"
            )
        ...
        try:
            ...
            for obsid in range(basic["nobservations"]):
                obs_info = {}
                obs_info["observatory"] = (
                    observatory_names[obsid]
                    if obsid < len(observatory_names)
                    else f"obs{obsid}"
                )
                obs_info["observer"] = (
                    observers[obsid] if obsid < len(observers) else f"observer{obsid}"
                )
                obs_info["project"] = (
                    projects[obsid] if obsid < len(projects) else f"project{obsid}"
                )
        ...
        try:
            ...
            for i, src_id in enumerate(source_ids):
                ...
                src_info["name"] = (
                    source_names[i] if i < len(source_names) else f"source{src_id}"
                )
        ...
        [
            ms_metadata["spectral_windows"][spw]["reffreq_hz"]
            for spw in range(basic["nspw"])
        ]
        [
            ms_metadata["spectral_windows"][spw]["bandwidth_hz"]
            for spw in range(basic["nspw"])
        ]
        center_frequencies = [
            ms_metadata["spectral_windows"][spw]["centerfreq_hz"]
            for spw in range(basic["nspw"])
        ]
        channels = [
            ms_metadata["spectral_windows"][spw]["nchan"]
            for spw in range(basic["nspw"])
        ]
        ...
        # EVLA bands
        bands = [
            ms_metadata["spectral_windows"][spw]["evla_band"]
            for spw in range(basic["nspw"])
        ]
        ...
        # Field information
        field_positions = [
            ms_metadata["fields"][fid]["phasecenter"] for fid in range(basic["nfields"])
        ]
        ...
        # Field-spw mapping
        field_spws = [
            ms_metadata["fields"][fid]["spws"] for fid in range(basic["nfields"])
        ]
        pipeline_context["field_spws"] = field_spws

        # Field-scan mapping
        field_scan_map = {
            fid: ms_metadata["fields"][fid]["scans"] for fid in range(basic["nfields"])
        }
        ...
        for intent in calibration_intents:
            ...
            if matched_fields:
                ...
                cal_fields[intent_key] = sorted(set(matched_fields))
                cal_scans[intent_key] = sorted(set(matched_scans))
                cal_strings[f"{intent_key}_field_select_string"] = ",".join(
                    map(str, cal_fields[intent_key])
                )
                cal_strings[f"{intent_key}_scan_select_string"] = ",".join(
                    map(str, cal_scans[intent_key])
                )
            else:
        ...
        pipeline_context["flux_field_select_string"] = cal_strings[
            "flux_field_select_string"
        ]
        pipeline_context["bandpass_field_select_string"] = (
            cal_strings["bandpass_field_select_string"]
            or cal_strings["flux_field_select_string"]
        )
        pipeline_context["delay_field_select_string"] = (
            cal_strings["delay_field_select_string"]
            or cal_strings["bandpass_field_select_string"]
            or cal_strings["flux_field_select_string"]
        )
        pipeline_context["phase_field_select_string"] = cal_strings[
            "phase_field_select_string"
        ]
        pipeline_context["amp_field_select_string"] = (
            cal_strings["ampli_field_select_string"]
            or cal_strings["phase_field_select_string"]
        )

        pipeline_context["flux_scan_select_string"] = cal_strings[
            "flux_scan_select_string"
        ]
        pipeline_context["bandpass_scan_select_string"] = (
            cal_strings["bandpass_scan_select_string"]
            or cal_strings["flux_scan_select_string"]
        )
        pipeline_context["delay_scan_select_string"] = (
            cal_strings["delay_scan_select_string"]
            or cal_strings["bandpass_scan_select_string"]
        )
        pipeline_context["phase_scan_select_string"] = cal_strings[
            "phase_scan_select_string"
        ]
        pipeline_context["amp_scan_select_string"] = (
            cal_strings["ampli_scan_select_string"]
            or cal_strings["phase_scan_select_string"]
        )

        pipeline_context["polarization_angle_field_select_string"] = cal_strings[
            "pol_angle_field_select_string"
        ]
        pipeline_context["polarization_angle_scan_select_string"] = cal_strings[
            "pol_angle_scan_select_string"
        ]
        pipeline_context["polarization_lkg_field_select_string"] = cal_strings[
            "pol_leakage_field_select_string"
        ]
        pipeline_context["polarization_lkg_scan_select_string"] = cal_strings[
            "pol_leakage_scan_select_string"
        ]
        ...
        if len(cal_fields["pol_angle"]) == 0 and do_pol:
            ...
            for polcal in polcals_A:
                ...
                if pol_angle_field_ids:
                    pipeline_context["polarization_angle_field_list"].extend(
                        pol_angle_field_ids
                    )
                    ...
                    pipeline_context["polarization_angle_scan_list"] = sorted(
                        set(pipeline_context["polarization_angle_scan_list"])
                    )
                    break
        ...
        if not pipeline_context.get("polarization_angle_field_list") and do_pol:
            do_pol = False
            task_logprint(
                "WARNING: No polarization angle calibrator found. Disabling polarization calibration."
            )
        ...
        if len(cal_fields["pol_leakage"]) == 0 and do_pol:
            ...
            for polcal in polcals_C:
                ...
                if pol_lkg_field_ids:
                    pipeline_context["polarization_lkg_field_list"].extend(
                        pol_lkg_field_ids
                    )
                    for field_id in pol_lkg_field_ids:
                        scans = msmd.scansforfield(field_id)
                        pipeline_context["polarization_lkg_scan_list"].extend(scans)
                    pipeline_context["polarization_lkg_scan_list"] = sorted(
                        set(pipeline_context["polarization_lkg_scan_list"])
                    )
                    break
        ...
        if not pipeline_context.get("polarization_lkg_field_list") and do_pol:
            do_pol = False
            task_logprint(
                "WARNING: No polarization leakage calibrator found. Disabling polarization calibration."
            )

        pipeline_context["do_pol"] = do_pol

        # Update polarization selection strings
        pipeline_context["polarization_angle_field_select_string"] = ",".join(
            map(str, pipeline_context.get("polarization_angle_field_list", []))
        )
        pipeline_context["polarization_angle_scan_select_string"] = ",".join(
            map(str, pipeline_context.get("polarization_angle_scan_list", []))
        )
        pipeline_context["polarization_lkg_field_select_string"] = ",".join(
            map(str, pipeline_context.get("polarization_lkg_field_list", []))
        )
        pipeline_context["polarization_lkg_scan_select_string"] = ",".join(
            map(str, pipeline_context.get("polarization_lkg_scan_list", []))
        )
        ...
        if not all_cal_fields:
            task_logprint(
                "WARNING: No calibrators found via intents, attempting to identify by field names"
            )
                ...
                for cal_name in standard_cal_names:
                    if cal_name in field_name_lower:
                        ...
                        try:
                            ...
                            task_logprint(
                                f"Identified calibrator by name: {field_name} (field {field_id})"
                            )
                        except:
                            pass
                        break
        ...
        pipeline_context["calibrator_field_list"] = sorted(all_cal_fields)
        pipeline_context["calibrator_scan_list"] = sorted(all_cal_scans)
        pipeline_context["calibrator_field_select_string"] = ",".join(
            map(str, sorted(all_cal_fields))
        )
        pipeline_context["calibrator_scan_select_string"] = ",".join(
            map(str, sorted(all_cal_scans))
        )

        task_logprint(
            f"Identified {len(all_cal_fields)} calibrator field(s): {pipeline_context['calibrator_field_select_string']}"
        )
        task_logprint(
            f"Identified {len(all_cal_scans)} calibrator scan(s): {pipeline_context['calibrator_scan_select_string']}"
        )
        ...
        if not pipeline_context.get("flux_field_select_string"):
            ...
            if calib_fields:
                ...
                if not pipeline_context.get("flux_field_list"):
                    pipeline_context["flux_field_list"] = sorted(calib_fields)
                # Set selection string from the (possibly new) flux_field_list
                pipeline_context["flux_field_select_string"] = ",".join(
                    map(str, pipeline_context["flux_field_list"])
                )
                task_logprint(
        ...
        cal3C84_d = (
            any(field_id in cal_fields["delay"] for field_id in fields_3C84_ids)
            if fields_3C84_ids
            else False
        )
        cal3C84_bp = (
            any(field_id in cal_fields["bandpass"] for field_id in fields_3C84_ids)
            if fields_3C84_ids
            else False
        )
        pipeline_context["cal3C84_d"] = cal3C84_d
        pipeline_context["cal3C84_bp"] = cal3C84_bp
        pipeline_context["uvrange3C84"] = (
            "0~1800klambda" if cal3C84_d or cal3C84_bp else ""
        )
        ...
        if len(spw_names) > 0 and "#" in spw_names[0]:
            ...
            for i, spw_name in enumerate(spw_names):
                ...
                if len(parts) == 3:
                    ...
                    bands_basebands_subbands.append(
                        [receiver_band, baseband, int(subband), i]
                    )
        ...
        if len(scan_numbers) > 1:
            ...
            for i in range(1, len(scan_numbers)):
                ...
                if (current_scan_fields != first_scan_fields) or (
                    current_scan_spws != first_scan_spws
                ):
                    scan_list.append(scan_numbers[i])
                    first_scan_fields = current_scan_fields
                    first_scan_spws = current_scan_spws
        pipeline_context["quack_scan_string"] = (
            ",".join(map(str, sorted(scan_list))) if scan_list else ""
        )

        # numSpws2 adjustment for pointing
        num_pointing_scans = len(cal_scans["pointing"])
        pipeline_context["numSpws2"] = (
            basic["nspw"] - 2 if num_pointing_scans > 0 else basic["nspw"]
        )
        ...
        task_logprint(
            f"Observation start date: {pipeline_context['startdate']:.2f} MJD"
        )
        ...
        msmd.close()
        task_logprint("Finished get_ms_info (using msmetadata)")
        task_logprint(
            f"QA2 score: {format_qa_status(pipeline_context.get('QA2_msinfo', 'Unknown'))}"
        )
        runtiming("msinfo", "end")
...
def EVLA_pipe_msmd(pipeline_context):
    ...
    if QA2_score == "Fail":
        raise RuntimeError(
            "MS metadata extraction failed - cannot proceed with pipeline"
        )

    task_logprint("Finished EVLA_pipe_msmd.py")
