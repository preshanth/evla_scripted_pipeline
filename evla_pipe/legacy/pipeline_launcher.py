...
class PipelineLauncher:
    ...
    def save_context(
        self, filename: Optional[str] = None, stage: Optional[str] = None
    ) -> str:
        """
        ...
        if skipped_keys:
            self.log(
                f"Skipping non-serializable context keys: {', '.join(skipped_keys)}"
            )
    ...
    def _validate_loaded_context(self):
        ...
        if metadata:
            ...
            save_date = datetime.datetime.fromtimestamp(save_time).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
    ...
    def _validate_data_files(self):
        ...
        if missing_files:
            self.log(
                f"WARNING: Missing expected files for resume: {'; '.join(missing_files)}"
            )
        else:
    ...
    def step_metadata_extraction(self):
        ...
        try:
            from evla_pipe.metadata_extraction import (
                extract_ms_metadata,
                identify_calibrators,
            )
    ...
    def show_status(self, context_file: Optional[str] = None):
        ...
        if self.context.get("step_metadata_complete", False):
            ...
            if cal_fields:
                ...
                print(
                    f"Total calibrator fields: {len(cal_fields.get('all_calibrators', []))}"
                )
        ...
        if progression:
            ...
            for i, summary in enumerate(progression):
                ...
                if i == 0:
                    print(
                        f"{i+1}. {operation}: {flagged_fraction:.4f} flagged (initial)"
                    )
                else:
                    print(
                        f"{i+1}. {operation}: {flagged_fraction:.4f} flagged (+{flags_added} flags)"
                    )

                # Show warnings if present
                if summary.get("over_flagging_warning", False):
                    print("    ⚠️  WARNING: Possible over-flagging detected!")
        else:
            print("\n=== Flagging Progression ===")
...
Examples:
  ...
  %(prog)s status                            # Show pipeline status
    ...
    run_parser.add_argument(
        "--hanning", action="store_true", help="Apply Hanning smoothing after import"
    )
