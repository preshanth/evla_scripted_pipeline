...
Examples:
  ...
  %(prog)s reset
    ...
    run_parser.add_argument(
        "--skip-hanning", action="store_true", help="Skip Hanning smoothing"
    )
    run_parser.add_argument(
        "--enable-polarization",
        action="store_true",
    ...
    run_parser.add_argument(
        "--disable-plots", action="store_true", help="Disable plotting"
    )
    run_parser.add_argument(
        "--resume", action="store_true", help="Resume from last checkpoint"
    )
    run_parser.add_argument(
        "--resume-from", metavar="STEP", help="Resume from specific step"
    )
    run_parser.add_argument("--stop-at", metavar="STEP", help="Stop at specific step")
    run_parser.add_argument(
        "--state-file", default="pipeline_state.json", help="State file path"
    )
    run_parser.add_argument(
    ...
    # Steps command
    subparsers.add_parser("steps", help="List available pipeline steps")
...
def cmd_run(args):
    ...
    try:
        ...
        print(
            f"Final progress: {progress['completed_steps']}/{progress['total_steps']} steps completed"
        )
...
def cmd_status(args):
    ...
    if state_manager.state["checkpoints"]:
        ...
        print(
            f"  Last checkpoint: {last_checkpoint['step_name']} ({last_checkpoint['status']})"
        )
        ...
        if resume_point:
            ...
            print("  Pipeline appears complete")
...
def cmd_reset(args):
    ...
    if not args.confirm:
        response = input("Are you sure you want to reset pipeline state? (y/N): ")
        if response.lower() != "y":
