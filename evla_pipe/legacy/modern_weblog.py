...
from evla_pipe.utils import get_log_path, get_weblog_path, logprint
...
class ModernWeblogGenerator:
    ...
    def generate_weblog(self):
        ...
        logprint(
            "*** Generating modern weblog ***",
            logfileout=str(get_log_path("weblog.log")),
        )
        ...
        logprint(
            "Modern weblog generation completed",
            logfileout=str(get_log_path("weblog.log")),
        )
    ...
    def _collect_qa_data(self):
        ...
        qa_steps = [
            ...
            (
                "QA2_hanning",
                "Hanning Smoothing",
                "Apply Hanning smoothing for RFI reduction",
            ),
            ("QA2_msinfo", "MS Information", "Extract measurement set metadata"),
            (
                "QA2_flagall",
                "Initial Flagging",
                "Apply online flags and basic RFI flagging",
            ),
            ("QA2_calprep", "Calibration Prep", "Prepare calibration setup and models"),
            (
                "QA2_priorcals",
                "Prior Calibrations",
                "Apply antenna position and gain curve corrections",
            ),
            (
                "QA2_testBPdcals",
                "Test Bandpass",
                "Initial bandpass and delay calibration",
            ),
            (
                "QA2_flag_baddeformatters",
                "Bad Deformatter Flagging",
                "Flag known bad deformatter issues",
            ),
            ("QA2_checkflag", "Flag Analysis", "Analyze flagging statistics"),
            (
                "QA2_semiFinalBPdcals1",
                "Semi-final Bandpass",
                "Refined bandpass calibration",
            ),
            (
                "QA2_checkflag_semiFinal",
                "Semi-final Flag Check",
                "Check flagging after bandpass",
            ),
            (
                "QA2_solint",
                "Solution Intervals",
                "Determine optimal calibration intervals",
            ),
            ...
            (
                "QA2_fluxboot",
                "Flux Bootstrapping",
                "Bootstrap flux scale to secondary calibrators",
            ),
            (
                "QA2_finalcals",
                "Final Calibrations",
                "Generate final calibration tables",
            ),
        ...
        self.weblog_data.update(
            {
                ...
                "total_flagged_percent": self.context.get(
    ...
    def _collect_polarization_data(self):
        ...
        pol_data = {
            ...
            "polmodel_status": (
                "pass" if self.context.get("polarization_models_set") else "partial"
            ),
            "polmodel_result": (
                "Pass" if self.context.get("polarization_models_set") else "Partial"
            ),
        ...
        pol_qa_content = self.template_engine.render_template(
            "polarization_qa", pol_data
        )
        self.weblog_data["polarization_qa_section"] = pol_qa_content

    def _generate_summary_page(self):
        """Generate the main summary page."""
        self.template_engine.render_template("summary", self.weblog_data)
    ...
    def _generate_observation_page(self):
        ...
        full_page = self.template_engine.generate_full_page(
            "Observation Details",
            "base",
            {**self.weblog_data, "content": obs_content},
            "observation",
        )
    ...
    def _generate_calibration_page(self):
        ...
        full_page = self.template_engine.generate_full_page(
            "Calibration Details",
            "base",
            {**self.weblog_data, "content": cal_content},
            "calibration",
        )
    ...
    def _generate_qa_page(self):
        ...
        self.template_engine.render_template("qa_report", qa_context)
        full_page = self.template_engine.generate_full_page(
    ...
    def _generate_plots_page(self):
        ...
        full_page = self.template_engine.generate_full_page(
            "Diagnostic Plots",
            "base",
            {**self.weblog_data, "content": plots_content},
            "plots",
        )
...
def EVLA_pipe_modern_weblog(pipeline_context):
    ...
    try:
        ...
        logprint(
            f"Error generating modern weblog: {e}",
            logfileout=str(get_log_path("weblog.log")),
        )
