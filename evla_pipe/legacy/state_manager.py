...
class PipelineStateManager:
    ...
    def _sanitize_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in context.items():
            if isinstance(value, safe_types):
                ...
                sanitized[f"__{key}__repr"] = repr(value)[
                    :200
    ...
    def _sanitize_list(self, lst: list) -> list:
        ...
        for item in lst:
            if isinstance(item, (str, int, float, bool, type(None))):
                ...
                sanitized.append(
                    {"__type": str(type(item)), "__repr": repr(item)[:100]}
                )
        return sanitized
    ...
    def save_checkpoint(
        ...
        step_index = (
            self.pipeline_steps.index(step_name)
    ...
    def get_pipeline_progress(self) -> Dict[str, Any]:
        ...
        completed_steps = len(
            [cp for cp in self.state["checkpoints"] if cp["status"] == "completed"]
        )
