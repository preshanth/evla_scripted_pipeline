...
from evla_pipe.utils import RefAntHeuristics, find_EVLA_band, logprint
...
class PolConfig:
    ...
    angle_calibrators: List[str] = field(
    ...
    leakage_calibrators: List[str] = field(
        default_factory=lambda: [
            "J0542+4951",
            "3c147",
            "3C147",
...
class PolarizationCalibrator:
    ...
    def solve_kcross(
        self, angle_calibrator: PolCalibrator, gaintables: List[str] = None
    ) -> str:
        """
    ...
    def solve_leakage(
        self, leakage_calibrator: PolCalibrator, gaintables: List[str] = None
    ) -> str:
        """
    ...
    def calibrate_polarization(
        self, calibrators: Dict[str, PolCalibrator], prior_tables: List[str] = None
    ) -> Dict[str, str]:
        """
        ...
        if self.config.solve_angle:
            polangle_table = self.solve_polarization_angle(
                calibrators["angle"], applied_tables
            )
...
def find_pol_calibrators(
    msname: str, config: Optional[PolConfig] = None
) -> Dict[str, PolCalibrator]:
    """
...
def solve_pol_kcross(
    msname: str,
    angle_calibrator: PolCalibrator,
    gaintables: List[str] = None,
    config: Optional[PolConfig] = None,
) -> str:
    """
...
def calibrate_polarization_full(
    msname: str, prior_tables: List[str] = None, config: Optional[PolConfig] = None
) -> Dict[str, str]:
    """
    Run complete polarization calibration sequence.
