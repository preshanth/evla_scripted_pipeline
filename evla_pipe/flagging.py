...
class PipelineFlagging:
    ...
    def _apply_flag_stage(self, commands: List[str], stage_name: str) -> Dict[str, Any]:
        ...
        start_time = runtiming(f"flagging_{stage_name.lower().replace(' ', '_')}", "start")
    ...
    def stage1_initial_flagging(self) -> Dict[str, Any]:
        ...
        bandpass_field_select_string = self.context.get("bandpass_field_select_string", "")
        delay_field_select_string = self.context.get("delay_field_select_string", "")
        calibrator_scan_select_string = self.context.get("calibrator_scan_select_string", "")
        ...
        if os.path.isfile(online_flag_name):
            commands.append(self._format_flag_command(
                mode='list', 
        ...
        commands.append(self._format_flag_command(
            mode='manual', 
            intent='SYSTEM_CONFIGURATION#UNSPECIFIED', 
            reason='setup'
        ))
    ...
    def stage2_calibrator_flagging(self) -> Dict[str, Any]:
        ...
        if checkflagfields and testgainscans:
            commands.append(f"mode='rflag' field='{checkflagfields}' correlation='ABS_{corrstring}' scan='{testgainscans}' ntime='scan' combinescans=False datacolumn='corrected' winsize=3 timedevscale=4.0 freqdevscale=4.0 extendflags=False reason='RFI_calibrators'")
    ...
    def stage3_semifinal_flagging(self) -> Dict[str, Any]:
        ...
        if checkflagfields and testgainscans:
            commands.append(f"mode='rflag' field='{checkflagfields}' correlation='ABS_{corrstring}' scan='{testgainscans}' ntime='scan' combinescans=False datacolumn='corrected' winsize=3 timedevscale=3.0 freqdevscale=3.0 extendflags=False reason='RFI_semifinal'")
    ...
    def stage4_target_flagging(self) -> Dict[str, Any]:
        ...
        commands.append(f"mode='rflag' field='' correlation='ABS_{corrstring}' scan='' intent='*CALIBRATE*' ntime='scan' combinescans=False datacolumn='corrected' winsize=3 timedevscale=4.0 freqdevscale=4.0 extendflags=False reason='RFI_all_calibrators'")
        
        # RFI flagging on all target scans  
        commands.append(f"mode='rflag' field='' correlation='ABS_{corrstring}' scan='' intent='*TARGET*' ntime='scan' combinescans=False datacolumn='corrected' winsize=3 timedevscale=4.0 freqdevscale=4.0 extendflags=False reason='RFI_all_targets'")
