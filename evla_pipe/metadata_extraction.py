...
import numpy as np
from casatasks import listobs, plotweather, rmtables, split
from casatools import ms, msmetadata, table

from evla_pipe.import_data import track_flagging_progression
from evla_pipe.utils import find_EVLA_band, get_log_path, logprint, runtiming
        ...
        if hasattr(msmd, "effexposuretime"):
            ...
            if isinstance(result, dict):
                ...
                if "value" in result:
                    ...
                    "m0" in result
                    and isinstance(result["m0"], dict)
                    and "value" in result["m0"]
                ):
                    return float(result["m0"]["value"])
                else:
                    task_logprint(
                        f"Unexpected effexposuretime dict structure: {result}"
                    )
                    return 0.0
...
def extract_ms_metadata(pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        task_logprint(
            f"MS: {basic_info['nspw']} spws, {basic_info['nfields']} fields, "
        ...
        for spwid in range(basic_info["nspw"]):
            ...
            if isinstance(ref_freq_dict, dict) and "m0" in ref_freq_dict:
                if (
                    isinstance(ref_freq_dict["m0"], dict)
                    and "value" in ref_freq_dict["m0"]
                ):
                ...
                reffreq_hz = (
                    float(ref_freq_dict)
                    if isinstance(ref_freq_dict, (int, float))
                    else 0.0
                )
            ...
            spw_info[spwid] = {
                ...
                "chanfreqs_hz": {
                    "data": (
                        chanfreqs.tolist()
                        if hasattr(chanfreqs, "tolist")
                        else list(chanfreqs)
                    ),
                    "type": "numpy_array",
                    "dtype": (
                        str(chanfreqs.dtype)
                        if hasattr(chanfreqs, "dtype")
                        else "float64"
                    ),
                },
                "chanwidths_hz": {
                    "data": (
                        chanwidths.tolist()
                        if hasattr(chanwidths, "tolist")
                        else list(chanwidths)
                    ),
                    "type": "numpy_array",
                    "dtype": (
                        str(chanwidths.dtype)
                        if hasattr(chanwidths, "dtype")
                        else "float64"
                    ),
                },
            }
        ...
        for field_id in range(basic_info["nfields"]):
            ...
            field_info[field_id] = {
                ...
                "direction": {
                    "data": (
                        direction if isinstance(direction, dict) else str(direction)
                    ),
                    "type": "casa_direction",
                },
                "intents": {
        ...
        for scan_id in msmd.scannumbers():
            ...
            scan_info[scan_id] = {
                "fields": {
                    "data": (
                        fields.tolist() if hasattr(fields, "tolist") else list(fields)
                    ),
                    "type": "numpy_array",
                ...
                "antennas": {
                    ...
                    "type": "numpy_array",
                    "dtype": (
                        str(antennas.dtype) if hasattr(antennas, "dtype") else "int32"
                    ),
                },
            }
        ...
        for i in range(basic_info["nantennas"]):
            ...
            diameters.append(
                float(diam) if isinstance(diam, (int, float)) else str(diam)
            )
        ...
        antenna_info = {
            "names": {
                "data": (
                    list(antenna_names)
                    if hasattr(antenna_names, "__iter__")
                    else [str(antenna_names)]
                ),
                "type": "string_list",
            },
            "positions": {"data": positions, "type": "casa_position_list"},
            "diameters": {"data": diameters, "type": "float_list"},
            "stations": {
                "data": (
                    stations.tolist() if hasattr(stations, "tolist") else list(stations)
                ),
                "type": "string_list",
            },
        }
        ...
        if (
            len(scan_numbers) > 0
            and basic_info["nspw"] > 0
            and basic_info["nobservations"] > 0
        ):
            try:
                exposure_time_dict = msmd.exposuretime(
                    scan=first_scan, spwid=0, obsid=first_obsid
                )
                if isinstance(exposure_time_dict, dict):
                    ...
                    elif "m0" in exposure_time_dict and isinstance(
                        exposure_time_dict["m0"], dict
                    ):
                        exposure_time = float(
                            exposure_time_dict["m0"].get("value", 0.0)
                        )
                    else:
                        task_logprint(
                            f"Unknown exposure time dict format: {exposure_time_dict}"
                        )
                        exposure_time = 0.0
                    ...
                    task_logprint(
                        f"Unknown exposure time type: {type(exposure_time_dict)}"
                    )
                    exposure_time = 0.0
            except Exception as e:
                task_logprint(
                    f"Could not get exposure time for scan={first_scan}, spwid=0, obsid={first_obsid}: {e}"
                )
                # Try to get from summary
                summary_exp = summary.get("ExposureTime", 0.0)
                exposure_time = (
                    float(summary_exp) if isinstance(summary_exp, (int, float)) else 0.0
                )
...
def calculate_tau(msname: str, start_time: float) -> float:
    ...
    try:
        ...
        if ((start_time >= 55918.80) and (start_time <= 55938.98)) or (
            ...
            task_logprint(
                "Weather station broken during this period, using 100% seasonal model"
            )
...
def identify_calibrators(pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if is_calibrator:
            ...
            for cal_type, intent_keywords in intent_mappings.items():
                if any(keyword in " ".join(intents) for keyword in intent_keywords):
                    ...
                    task_logprint(
                        f"Field {field_id} ({field_name}) identified as {cal_type} calibrator"
                    )

    # Create selection strings
    cal_strings = {}
    field_names = [
        pipeline_context["field_info"][fid]["name"]
        for fid in range(pipeline_context["numfields"])
    ]

    for cal_type, field_ids in cal_fields.items():
        if field_ids:
            # Create comma-separated string of field IDs
            cal_strings[f"{cal_type}_field_select_string"] = ",".join(
                map(str, field_ids)
            )
    ...
    calibrator_scans = sorted(set(calibrator_scans))
    ...
    pipeline_context.update(
        {
            ...
            "calibrator_scan_select_string": cal_strings[
                "calibrator_scan_select_string"
            ],
            "calibrator_field_select_string": cal_strings[
                "all_calibrators_field_select_string"
            ],
        }
    )
    ...
    task_logprint(
        f"Flux: {len(cal_fields['flux'])}, Bandpass: {len(cal_fields['bandpass'])}, "
...
def split_calibrators(pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    runtiming("split_calibrators", "start")
    ...
    pipeline_context = track_flagging_progression(
        pipeline_context, "before_calibrator_split"
    )
...
def extract_metadata_and_calibrators(
...
def print_metadata_summary(pipeline_context: Dict[str, Any]):
    ...
    if start_time and end_time:
        ...
        task_logprint(
            f"Observation Duration: {duration_hours:.2f} hours ({duration_sec:.0f} seconds)"
        )
    ...
    for scan_id in sorted(scan_info.keys()):
        ...
        if isinstance(times_data, dict) and "data" in times_data:
            ...
            times = (
                np.array(times_data) if isinstance(times_data, list) else np.array([])
            )
        ...
        task_logprint(
            f"{scan_id:<5} {fields_str:<15} {intent_str:<30} {duration_str:<10}"
        )
...
def print_comprehensive_summary(pipeline_context: Dict[str, Any]):
    ...
    for field_id, field_data in field_info.items():
        if isinstance(field_data, dict):
            ...
            scans = (
                get_numpy_array(field_data, "scans").tolist()
                if "scans" in field_data
                else []
            )
    ...
    for scan_id, scan_data in scan_info.items():
        if isinstance(scan_data, dict):
            fields = (
                get_numpy_array(scan_data, "fields").tolist()
                if "fields" in scan_data
                else []
            )
