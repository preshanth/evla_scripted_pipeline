...
from casatasks import setjy, listobs
from casatools import table, quanta
from evla_pipe.utils import (
    runtiming, logprint, format_qa_status, get_log_path,
    get_caltable_path, ensure_dir_exists
)
from evla_pipe.metadata_extraction import get_numpy_array, get_string_list
...
def perform_precalibration_setup(pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    start_time = runtiming("precalibration", "start")
        ...
        if pol_results.get("success", False):
            ...
            for field_id, model in pol_results.get("models", {}).items():
                ...
                if "position_angle" in model:
                    ...
                    task_logprint(f"Field {field_id} ({field_name}): position angle = {pa:.1f} deg")
                if "polarization_fraction" in model:
                    pf = model["polarization_fraction"] 
                    task_logprint(f"Field {field_id} ({field_name}): polarization fraction = {pf:.3f}")
...
def select_reference_antenna(pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(antenna_names_data, dict) and "data" in antenna_names_data:
        ...
        antenna_names = antenna_names_data if isinstance(antenna_names_data, list) else []
    ...
    preferred_antennas = [
        'ea13', 'ea14', 'ea15', 'ea16', 'ea17', 'ea18', 'ea19', 'ea20', 'ea21',  # Inner A-array
        'ea02', 'ea03', 'ea04', 'ea05', 'ea06', 'ea08', 'ea09', 'ea10', 'ea12'   # Good antennas
    ]
    ...
    for scan_id, scan_data in scan_info.items():
        if isinstance(scan_data, dict):
            ...
            if isinstance(times_data, dict) and "data" in times_data:
                ...
                if len(times) > 1:
                    scan_length = (times.max() - times.min()) * 86400  # Convert to seconds
