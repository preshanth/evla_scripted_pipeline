...
from casatasks import importasdm, hanningsmooth, flagdata
from casatools import table
from evla_pipe.utils import (
    runtiming, logprint, format_qa_status,
    get_measurement_set_path, cleanup_import_files,
    MEASUREMENT_SETS_DIR, get_log_path
)
...
def get_flagging_summary(ms_name: str, operation: str = "initial") -> Dict[str, Any]:
    ...
    try:
        ...
        task_logprint(f"Flagging summary ({operation}): {flagged_fraction:.4f} "
...
def validate_fresh_ms_flagging(ms_name: str) -> bool:
    ...
    if flagged_fraction > 0.15:  # 15% threshold
        task_logprint(f"WARNING: Fresh MS has high flagging fraction: {flagged_fraction:.4f}")
        task_logprint("This may indicate issues with the data or import process")
        return False
    else:
        task_logprint(f"Fresh MS flagging validation passed: {flagged_fraction:.4f} flagged")
        return True


def track_flagging_progression(pipeline_context: Dict[str, Any], 
    ...
    if progression:
        ...
        current_summary["previous_operation"] = previous_summary.get("operation", "unknown")
        ...
        if flags_added_fraction > 0.1:  # More than 10% flagged in one step
            task_logprint(f"WARNING: {operation} flagged {flags_added_fraction:.4f} "
...
def import_asdm_data(pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    start_time = runtiming("import", "start")
...
def apply_hanning_smooth(pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    start_time = runtiming("hanning", "start")
...
def import_with_optional_hanning(pipeline_context: Dict[str, Any], 
                                apply_hanning: bool = False) -> Dict[str, Any]:
    """
    ...
    if apply_hanning:
        ...
        if pipeline_context.get("QA2_hanning") == "Pass":
            pipeline_context = track_flagging_progression(pipeline_context, "post_hanning")
    else:
    ...
    if "flagging_progression" in pipeline_context:
        ...
        if progression:
            ...
            task_logprint(f"Final flagging state: {latest['flagged_fraction']:.4f} flagged")
...
def perform_data_import(pipeline_context: Dict[str, Any], 
                       apply_hanning: bool = False) -> Dict[str, Any]:
    """
    Convenience function for pipeline integration.
