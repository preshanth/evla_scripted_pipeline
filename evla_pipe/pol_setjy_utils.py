...
def load_polcal_data_from_file(source_name: str, data_dir: str = None) -> PolCalData:
    ...
    if source_upper not in source_files:
        raise ValueError(
            f"Unknown source {source_name}. Available: {list(source_files.keys())}"
        )
    ...
    with open(file_path, "r") as f:
        for line in f:
            ...
            if len(parts) >= 4:
                try:
                    freq = float(parts[0])
                    float(parts[1])  # Not used but in file
...
def load_perley_butler_2013_data(source_name: str, data_dir: str = None) -> PolCalData:
    ...
    if source_upper not in source_columns:
        ...
        raise ValueError(
            f"Source {source_name} not in Perley-Butler 2013 data. Available: {available}"
        )
    ...
    with open(file_path, "r") as f:
        for line in f:
            ...
            if (
                ...
                or line.startswith("Polarization")
                or line.startswith("Frequency")
                or line.startswith("(GHz)")
                or not line
            ):
                continue
            ...
            if len(parts) >= 9:  # Ensure we have all columns
                try:
                    ...
                    if (
                        "sdotsdotsdot" in pol_frac_str
                        or "<" in pol_frac_str
                        or "sdotsdotsdot" in pol_angle_str
                        or pol_frac_str == ""
                        or pol_angle_str == ""
                    ):
                        continue
    ...
    if not frequencies:
        raise ValueError(
            f"No valid data found for {source_name} in Perley-Butler 2013 table"
        )

    # Rotation measure values from Perley-Butler 2013
    rm_values = {"3C48": -68.0, "3C138": 0.0, "3C147": 0.0, "3C286": 0.0}
...
def get_polcal_data(
    source_name: str, data_dir: str = None, obs_date: str = None
) -> PolCalData:
    """
    ...
    if obs_date:
        try:
            ...
            logprint(
                f"Warning: Could not parse observation date '{obs_date}', using 2019 data"
            )
            use_2019_data = True
    ...
    if use_2019_data:
        try:
            ...
            logprint("Falling back to Perley-Butler 2013 data")
        ...
        try:
            ...
            logprint(
                f"Warning: Could not load Perley-Butler 2013 data for {source_name}: {e}"
            )
            logprint("Falling back to 2019 data")
            return load_polcal_data_from_file(source_name, data_dir)
...
def filter_data_for_band(
    cal_data: PolCalData, band: str
    ...
    if not np.any(mask):
        ...
        logprint(
            f"Warning: No polarization data for {cal_data.source_name} in {band} band"
        )
        return None, None, None

    return (
        cal_data.frequencies[mask],
        cal_data.pol_fraction[mask],
        cal_data.pol_angle[mask],
    )


def fit_polarization_polynomials(
    source_name: str, band: str, ref_freq_ghz: float, order: int = 3
    ...
    if len(freqs) < order + 1:
        logprint(
            f"Warning: Only {len(freqs)} points for {source_name} {band}-band, reducing polynomial order"
        )
...
def compute_stokes_qu(
    pol_fraction: float, pol_angle_rad: float, stokes_i: float
) -> Tuple[float, float]:
    """
...
def setjy_with_polarization(
    vis: str,
    field: str,
    spw: str = "",
    ref_freq_hz: float = None,
    stokes_i: float = None,
    spectral_index: List[float] = None,
    ...
    setjy_params = {"vis": vis, "field": field, "usescratch": use_scratch}
    ...
    if pol_fraction_coeffs is None:
        ...
        if stokes_i is not None:
            ...
            if pol_angle_coeffs is not None:
                ...
                stokes_q, stokes_u = compute_stokes_qu(
                    pol_frac_ref, pol_angle_ref, stokes_i
                )
...
def integrate_polarization_setjy(
    vis: str,
    field_id: int,
    field_name: str,
    spws: List[int],
    ...
    try:
        ...
        if pol_frac_coeffs is None:
            ...
            raise ValueError(
                f"No polarization data available for {field_name} in {band} band"
            )
        ...
        logprint("Falling back to intensity-only model")
