...
def _bootstrap_flux(pipeline_context, flux_field_select_string):
...
def _fit_power_law(fluxscale_result, center_frequencies):
        ...
        for band in np.unique(bands):
            ...
            if len(lfds) <= 2:
                ...
                try:
                    ...
                    task_logprint(f"Error during power law fitting for {source} {band}: {e}")
                    aa = 0.0
                    bb = 0.0
                    SNR = 0.0
...
def _set_fitted_flux(pipeline_context, fitting_results, scratch):
...
def _plot_flux_model(pipeline_context):
